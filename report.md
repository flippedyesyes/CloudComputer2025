...
### 2.1 痛点场景定义
在传统的自学场景（如准备期末考试或考证）中，学生面临三大痛点：
1.  **"学完即忘"**：缺乏即时、客观的效果评估手段，学习者难以确认自己是否真正掌握了知识点。
2.  **"盲点隐形"**：错误往往被忽略，无法像考试一样暴露知识盲区，导致复习效率低下。
3.  **"纠偏滞后"**：缺乏类似助教的角色提供实时反馈和针对性练习，错题难以转化为得分点。
...
### 2.3 关键技术指标
为了保证系统的可用性与工程价值，我们设定了以下量化指标：
*   **出题覆盖率**：生成的题目需覆盖上传资料中 top-5 核心概念的 100%。
*   **判卷准确性**：对于客观题实现 100% 准确判定，主观题解析需包含得分点说明。
*   **系统响应**：API 接口响应时间 < 200ms，异步任务（出题/判卷）均在后台队列处理，保证前端无阻塞。
